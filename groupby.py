...
personeller = {
    "Calışanlar":["Ahmet Yılmaz","Can Ertürk","Hassan Korkmaz","Cenk Saymaz","Ali Turan","Rıza Ertürk","Mustafa Can"],
...
result = df.groupby("Semt")["Maaş"].sum()
result = df.groupby("Semt")["Yaş"].mean()
result = df.groupby("Semt")["Maaş"].mean()
result = df.groupby("Semt")["Maaş"].sum()
result = df.groupby("Semt")["Calışanlar"].count()
